...
int    symtab_level;                    /* current level in the stack i.e 0 if we're working with the global symboltable, non-zero otherwise*/


/*Initialize the symbol table stack. Called on shell startup.Does not return. If there is an error, the shell exits.*/
...
/*Alloc memory for a new symbol table structure and give it the passed level.Returns a pointer to the newly alloc'ed symbol table. Doesn't return in case of error, as this function calls alloc_hash_table() and the latter doesn't return on error.*/
...
/*Push a symbol table on top the stack and assigns the newly added table as the local symbol table.*/
...
/*Create an empty symbol table and push on top the stack.Returns the newly pushed symbol table.*/
...
/*Pop the symbol table on top the stack, which we use as the local symbol table. This happens when we finish executing a builtin utility or shell function, in order to exit the local scope and return to the global scope.Returns the popped symbol table, or NULL if the stack is empty.*/
...
/*Release the memory used to store a symbol table structure, as well as the memory used to store the strings of key/value pairs we have stored in the table.*/
...
/*Add a string to a symbol table. Creates an entry for the string, adds it to the table, and then returns the new entry. In case of insufficient memory,this function exits the shell instead of returning NULL.*/
...
/*Add a string to the local symbol table. First checks if the string is already in the table or not, so as not to duplicate an entry. If not, creates an entryfor the string, adds it to the table, and then returns the new entry. In case of insufficient memory, this function exits the shell instead of returning NULL */
/*This function adds a new entry to the local symbol table. Rememberthat each entry must have a unique key, which is the name we give to the shell variable or function. To ensure this uniqueness, we first check to see if an entry exists with the given name, by calling dolookup*/
...
/*Search for a string in a symbol table. Returns the entry for the given string, or NULL if its not found.*/
...
/*Search for a string in the local symbol table.Returns the entry for the given string, or NULL if its not found.*/
...
/* Search for a string in the symbol table stack, starting at the top (the local symbol table), and checking every table, in turn, until we reach the bottom (the global symbol table. If the entry is found at any level (any symbol table in the stack), this entry is returned and the search stops. Otherwise, we checkthe table at the higher level, and so on until we reach the global symbol table.Returns the entry for the given string, or NULL if its not found.*/
...
/*Return a pointer to the local symbol table (this changes as we change scope by calling functions and builtin utilities).*/
...
/* Return a pointer to the global symbol table (this stays the same as long as the shell is running).*/
